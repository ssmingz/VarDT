 ...
 * @date 2021/12/25
 */
public class InstrumentForFilter {
    private String _name = "@InstrumentForFilter ";
    ...
    public void run() {
        ...
        File aMethods = new File(Constant.TOPN_METHODS_PATH);
        if(!aMethods.exists()) {
            JavaLogger.error(_name + "#run TopN methods file " + Constant.TOPN_METHODS_PATH + " not found");
            return;
        }
    ...
    public void execute(CompilationUnit cu) {
        ...
        GenStatement.generateForFilterVersion(cu, Constant.PROJECT_PATH);
    }

    /**
     * copy InstrAux.java to the folder containing the target
     * @param targetPath
     */
    private void copyAuxClass(String targetPath, PackageDeclaration pdToAdd) {
    ...
    public static void instrumentTestMethod(File testFile, Set<String> testMethods, String projectPath) {
        ...
        TestMethodVisitor visitor = new TestMethodVisitor(cu, testMethods, projectPath,testFile.getAbsolutePath());
        HashSet<MethodDeclaration> methods = visitor.traverse();
        // modify target AST
        GenStatement.genForTestMethod_filterVersion(methods);
    ...
    public void checkMethods() {
        JavaLogger.info("Start methods checking ...");

        // { clazz : [method] }
        Map<String, List<String>> methods_by_clazz = new LinkedHashMap<>();
        List<String> method_ids = new ArrayList<>();
        String writebuffer = "";
        try {
            FileReader reader = new FileReader(Constant.TOPN_METHODS_PATH);
            ...
            while((l = bReader.readLine()) != null) {
                ...
                if(methods_by_clazz.containsKey(filename)) {
                    tmp.addAll(methods_by_clazz.get(filename));
                }
                tmp.add(method);
                methods_by_clazz.put(filename, tmp);
        ...
        // instrument by file
        for(String clazz : methods_by_clazz.keySet()) {
            ...
            File propertiesFile = new File(propertiesPath);
            if(!propertiesFile.exists()) {
                JavaLogger.error(_name + "#checkMethods File doesn't exist : " + propertiesFile.getAbsolutePath());
                return;
            }
            String propertiesStr = JavaFile.readFileToString(propertiesFile);
            String[] lines = propertiesStr.split("\n");
            ...
            if(srcDir == null) {
                JavaLogger.error(_name + "#checkMethods srcDir not found in defects4j.build.properties : " + propertiesFile.getAbsolutePath());
                return;
            }
            ...
            for(String method : methods_by_clazz.get(clazz)) {
