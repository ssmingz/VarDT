...
public class Coverage {
	...
	public static Map<String, CoverInfo> computeOriginalCoverage(Subject subject,
        ...
        TraceTestMethodInstrumentVisitor newTestMethodInstrumentVisitor = new TraceTestMethodInstrumentVisitor(failedTestAndCoveredMethods.getFirst());
        ...
        for (Integer methodID : testcases) {
			...
			if (method == null) {
				LevelLogger.error(__name__ + "#collectCoveredMethod method string format error : " + Identifier.getMessage(methodID));
				continue;
			}
			String singleTest = method.getClazz() + "::" + method.getName();
			boolean success = Runner.testSingleCase(subject, singleTest);
			if (!success) {
				LevelLogger.error(__name__ + "#collectCoveredMethod build subject failed when running single test case.");
				continue;
			}
		}
